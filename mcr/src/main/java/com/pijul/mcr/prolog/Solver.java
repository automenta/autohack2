...
public class Solver {
    private final List<Clause> knowledgeBase;
    private final ToolProvider toolProvider;

    public Solver(List<Clause> knowledgeBase, ToolProvider toolProvider) {
    ...
    private void solve(Term query, Map<Variable, Term> substitution, List<Map<Variable, Term>> solutions) {
        ...
        for (Clause clause : knowledgeBase) {
            Map<Variable, Term> newSubst = Unification.unify(query, clause.getHead(), new HashMap<>(substitution));
            if (newSubst != null) {
                if (clause.isFact()) {
                    solutions.add(newSubst);
                } else {
                    solveBody(clause.getBody(), newSubst, solutions);
                }
            }
        }
    }
