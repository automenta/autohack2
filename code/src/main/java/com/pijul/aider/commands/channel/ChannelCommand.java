    ...
    public void execute(String[] args) {
        ...
        try {
            ...
            if (backend != null) {
                switch (subcommand) {
                    case "new":
                        container.getMessageHandler().addMessage("system", "Creating channel " + name);
                        // Actual implementation would call backend.channel("new", name);
                        break;
                    case "switch":
                        container.getMessageHandler().addMessage("system", "Switching to channel " + name);
                        // Actual implementation would call backend.channel("switch", name);
                        break;
                    case "list":
                        // Actual implementation would retrieve channels from backend
                        container.getMessageHandler().addMessage("system", "Channels: [main, feature-1]");
                        break;
                    default:
